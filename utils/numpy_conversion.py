            ...
            for i, eq in enumerate(system):
                ...
                if not eq[1]:
                    ...
                    for term in eq[1]:
                        ...
                        #term_str = term_str.replace('Derivative(x_1, (x, 2))', "np.gradient(np.gradient(x_1, axis=0))")
                        #term_str = term_str.replace('Derivative(x_2, (x, 2))', "np.gradient(np.gradient(x_2, axis=0))")
    ...
    with open(filename, 'w') as f:
        ...
        for n, system in enumerate(systems):
            ...
            for i, eq in enumerate(system):
                ...
                f.write(f"def eq_{n}_{i}(X, betas, t, derivatives):\n")
                f.write(f"    # d{var}/dt = {eq[1]}\n")
                f.write("    ")
                f.write(", ".join([f"x_{j + 1}" for j in range(M)]))  # Using j+1 to start from x_1
                f.write(" = X\n")
                ...
                if not eq[1]:
                    ...
                    for term in eq[1]:
                        ...
                        # Replace derivatives with the provided derivatives
                        term_str = term_str.replace('Derivative(x_1(t, x_1, x_2), (x_1, 2))', "derivatives['du_dxx']")
                        term_str = term_str.replace('Derivative(x_1(t, x_1, x_2), (x_2, 2))', "derivatives['du_dyy']")
                        term_str = term_str.replace('Derivative(x_2(t, x_1, x_2), (x_1, 2))', "derivatives['dv_dxx']")
                        term_str = term_str.replace('Derivative(x_2(t, x_1, x_2), (x_2, 2))', "derivatives['dv_dyy']")
            ...
            f.write(f"def system_{n}(X, betas, t, derivatives):\n")
            f.write(
                "    return np.array([" + ", ".join(f"eq_{n}_{i}(X, betas, t, derivatives)" for i in range(len(system))) + "])\n\n")
